...
use tracing::{error, event, info, span, trace, warn, Level};
...
fn load_rustls_config(ssl_config: &Ssl) -> rustls::ServerConfig {
    ...
    rustls::crypto::aws_lc_rs::default_provider()
        .install_default()
        .unwrap_or_else(|_| {
            error!("Failed to install default provider");
            panic!("Failed to install default provider");
        });
    trace!("Default AWS provider installed");
    ...
    trace!("Cert file loaded");
    let key_file = &mut BufReader::new(File::open(&ssl_config.key_file).unwrap_or_else(|_| {
        error!("Failed to open key file: {}", ssl_config.key_file.display());
        panic!("Failed to open key file: {}", ssl_config.key_file.display());
    }));

    trace!("Certificate keys loaded");
    // convert files to key/cert objects
    let cert_chain = certs(cert_file)
        .collect::<Result<Vec<_>, _>>()
        .unwrap_or_else(|_| {
            error!("Failed to parse certificate chain.");
            panic!("Failed to parse certificate chain.");
        });

    trace!("Certificate chain loaded");
    let key = pkcs8_private_keys(key_file)
        ...
        .unwrap_or_else(|_| {
            error!("Failed to parse private keys.");
            panic!("Failed to parse private keys.");
        });
